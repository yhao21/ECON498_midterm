...
class Parsing():
    ...
    def parsing_coin_html(self):
        ...
        for one_file in glob.glob(self.file_path + '/*.html'):
            ...
            try:
                rows = soup.find('tbody',{'class':'rc-table-tbody'}).find_all('tr', \
    ...
    def get_url_name(self, link_base, web_id):
        '''
        extract url_name from each currency deeplink url
        '''

        if web_id == '1':
            fname = re.compile(r'/en/coins/(.*)').findall(link_base)[0].replace(' ', '').lower()
        elif web_id == '0':
            fname = re.compile(r'/currencies/(.*)').findall(link_base)[0].replace(' ','').lower()
