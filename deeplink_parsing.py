...
class ParsingDeepLink():
    ...
    def save_to_log(self, ID):
        ...
        if ID == 'coinmkt':
            deeplink_id = 'coinmarketcap'
        elif ID == 'gecko':
            deeplink_id = 'gecko'

        df = pd.read_csv('500deeplinks.csv')
        missing_coin = df.loc[df['name'] == self.fname]
        # url is a list contain elements, each is a list.
        #[[2 'ethereum' 'https://coinmarketcap.com/currencies/ethereum' ]
        #[3 'ethereum' 'https://www.coingecko.com/en/coins/ethereum' ]] 
        url = missing_coin.loc[missing_coin['deeplinks'].str.contains(deeplink_id)].values
        url = missing_coin.loc[missing_coin['deeplinks'].str.contains(deeplink_id)].values[-1][-1]
