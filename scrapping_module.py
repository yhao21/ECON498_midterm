...
def recover_error_deeplink(folder):
    ...
    # go through each url in Error_Log.csv. variable 'currency' is one line
    # ['aave-eth' 'https://www.coingecko.com/en/coins/aave-eth']
    for currency in df:

        # if url does not contain 'gecko', it is coinmkt's url
        # program will set folder = 'coin_500deeplink'
        if str(re.compile(r'gecko').findall(currency[1])) != '[]':
            print(currency[1])
            fname = currency[0]
            deeplink_url = currency[1]
            #scrapping_error_page(folder[0], fname, deeplink_url)

        elif str(re.compile(r'coinmarketcap').findall(currency[1])) != '[]':
            print(currency[1])
            fname = currency[0]
            deeplink = currency[1]
            #scrapping_error_page(folder[1], fname, deeplink_url)
...
def scrapping_error_page(folder, name, url):
    ...
    # test
    one_file = os.path.join(folder_path, name + '.html')
